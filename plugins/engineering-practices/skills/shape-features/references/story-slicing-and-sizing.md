...
**"We can't ship half a feature." You can - decouple *deployment* from *release*.** The most common objection to 2-day production slices is that the increment is not user-ready. Two standard mechanisms resolve it; pick by the kind of change:

- **Feature flags / dark launches** - for *new* user-facing behavior. Ship the slice to production behind a flag (visible to nobody, or only to the team), integrate and verify it in the real environment, and flip it on when enough slices add up to something users should see. If your platform has no flagging tooling, a config toggle or an allowlist of internal users does the job. One discipline: note the flag's removal as deferred scope - flags that outlive their release become permanent complexity.
- **Branch by abstraction** - for *replacing existing* behavior (swapping a component, migrating a data store, rewriting an integration), where there is nothing to toggle on for users. Introduce an abstraction seam in front of the old implementation, build the new implementation behind the same seam slice by slice, move consumers over gradually, then remove the old implementation - and the seam itself, once it no longer earns its keep. Every slice ships to production from the main branch; nothing accumulates in a long-lived feature branch.

Either way, the requirement stays a vertical slice; the mechanism is what makes shipping it safe before it is user-visible.
